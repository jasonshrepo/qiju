...
def _make_qiju_project(path, slug, agents):
    import json
    path.mkdir(parents=True, exist_ok=True)
    qd = path / ".qiju"
    qd.mkdir()
    cfg = {"schema_version": 1, "project": slug, "enabled_agents": agents}
    (qd / "config.json").write_text(json.dumps(cfg), encoding="utf-8")
    return path


def test_uninstall_discovers_registry_only_project(qiju_env, tmp_path, monkeypatch):
    """A project registered but outside standard scan roots must be cleaned by uninstall."""
    # Put the project somewhere the scan would never find it
    project = _make_qiju_project(tmp_path / "hidden-project", "hidden", ["claude"])
    claude_skills = project / ".claude" / "skills" / "qiju-log"
    claude_skills.mkdir(parents=True)
    (claude_skills / "SKILL.md").write_text("content", encoding="utf-8")

    register.register_project(project, "hidden")

    result = cleanup.cleanup(
        user=False,
        project_root=None,
        hosts=("claude",),
        scan_projects=False,   # no scan — relies purely on registry
        dry_run=False,
    )

    assert not (claude_skills / "SKILL.md").exists()
    assert any(str(project) in a.path for a in result.actions)


def test_uninstall_unregisters_cleaned_project(qiju_env, tmp_path, monkeypatch):
    project = _make_qiju_project(tmp_path / "proj", "proj", ["claude"])
    ...
    assert str(project.resolve()) not in projects_map


def test_uninstall_dry_run_does_not_unregister(qiju_env, tmp_path, monkeypatch):
    project = _make_qiju_project(tmp_path / "proj", "proj", ["claude"])
    register.register_project(project, "proj")
