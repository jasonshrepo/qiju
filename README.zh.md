...
```bash
# 1. 从源码安装（把 `qiju` 装到 ~/.local/bin，引擎装到 ~/.qiju/qiju）
...
方式见 [使用 QiJu](#使用-qiju)，安装细节见 [从源码安装](#从源码安装)。
...
QiJu 是一个**源码优先的开发者预览版**。你从本仓库安装；目前还没有公开的包管理器发行版。
...
- **仅支持源码安装。** 你用 `bash install.sh` 从本仓库安装。目前没有 `pip install qiju`、
  `brew install qiju` 或 `npm install -g qiju`。
...
> 目前仅支持源码安装。没有 `npm install -g qiju`、`brew install qiju` 或 `pip install qiju`。
